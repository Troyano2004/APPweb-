  ...
  template: `
    <section class="page">
      <div class="page-header">
        ...
        <button class="btn-primary" (click)="abrirModal(null)">+ Nueva Facultad</button>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let item of items">
              ...
              <td>{{ item.nombre }}</td>
              <td>{{ item.nombreUniversidad || '-' }}</td>
              <td>
                <button class="btn-small btn-edit" (click)="abrirModal(item)">Editar</button>
                <button class="btn-small btn-delete" (click)="eliminar(item.idFacultad!)">Eliminar</button>
              </td>
            </tr>
      ...
      <div class="modal" *ngIf="modalAbierto" (click)="cerrarModal()">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h2>{{ editando ? 'Editar' : 'Nueva' }} Facultad</h2>
          ...
          <form (ngSubmit)="guardar()" class="modal-body">
            ...
            <div class="form-group">
              ...
              <select [(ngModel)]="formulario.idUniversidad" name="idUniversidad" required class="form-control">
...
export class FacultadComponent implements OnInit {
  ...
  formulario: Facultad = this.nuevoFormulario();
  modalAbierto = false;
  editando = false;
  loading = false;
  guardando = false;
  ...
  cargarUniversidades(): void {
    this.service.listarUniversidades().subscribe({
      ...
      error: () => { this.error = 'Error al cargar universidades'; }
    });
  }

  abrirModal(item: Facultad | null): void {
  ...
  guardar(): void {
    if (!this.formulario.nombre || !this.formulario.idUniversidad) {
      this.error = 'Complete campos obligatorios';
      return;
    }
    this.guardando = true;
    const op = this.editando
      ? this.service.actualizarFacultad(this.formulario.idFacultad!, this.formulario)
      : this.service.crearFacultad(this.formulario);
    op.subscribe({
      next: () => {
  ...
  private nuevoFormulario(): Facultad {
    return { nombre: '', idUniversidad: 0 };
  }
}
