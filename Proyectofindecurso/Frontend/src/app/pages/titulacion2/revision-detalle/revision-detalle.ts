...
export class RevisionDetalle implements OnInit {
  ...
  // ✅ Computed signal que parsea el JSON de la IA
  parsedAiResult = computed<AiResultParsed | null>(() => {
    const raw = this.aiResult();
    if (!raw) return null;
    try {
  ...
  exportarPdf(): void {
    ...
    const logoUrl = 'logo.png'; // ✅ ruta relativa, sin origin
    ...
    fetch(logoUrl, { cache: 'force-cache' })
      ...
      .catch(() => '')
      .then((logoBase64: string) => {
        this.generarImpresion(doc, logoBase64);
      });
  }


  private generarImpresion(doc: DocumentoTitulacionDto, logoBase64: string): void {
    ...
    // ✅ Usa base64 si se cargó, si no muestra texto alternativo
    const logoHtml = logoBase64
      ? `<img src="${logoBase64}" alt="Logo UTEQ" />`
      : `<span style="font-size:14pt;font-weight:700;">UTEQ</span>`;
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: 'Times New Roman', Times, serif; color: #121212; font-size: 12pt; line-height: 1.6; margin: 0; }
        .cover-page { min-height: calc(100vh - 4.4cm); display: flex; flex-direction: column; align-items: center; justify-content: space-between; text-align: center; page-break-after: always; padding: 0.3cm 0 0.8cm; }
        .cover-logo { margin-bottom: 0.3cm; }
        .cover-logo img { width: 110px; height: 110px; object-fit: contain; }
        .cover-university { font-size: 18pt; font-weight: 700; letter-spacing: 0.02em; text-transform: uppercase; margin: 0; }
        .cover-faculty { margin: 0.15cm 0 0; font-size: 12.2pt; }
        .cover-career { margin: 0.05cm 0 0; font-size: 11.5pt; }
        .cover-divider { width: 100%; border: 0; border-top: 1.5px solid #4f4f4f; margin: 0.9cm 0; }
        .cover-title { margin: 0; font-size: 20pt; font-weight: 700; text-transform: uppercase; }
        .cover-subtitle { margin: 0.28cm 0 0; font-size: 13pt; }
        .cover-box { text-align: left; width: 100%; max-width: 14cm; font-size: 12pt; line-height: 1.55; }
        .cover-box strong { font-weight: 700; }
        .cover-footer { font-size: 12pt; text-align: center; }
        .doc-main { position: relative; padding-bottom: 2cm; }
        .doc-header { text-align: center; border-bottom: 2px solid #1d7f43; margin-bottom: 0.9rem; padding-bottom: 0.5rem; }
        .doc-header h1 { margin: 0; font-size: 15.5pt; text-transform: uppercase; color: #14532d; }
        .doc-meta { margin-top: 0.22rem; font-size: 10.8pt; color: #303030; }
        .doc-section { break-inside: avoid; page-break-inside: avoid; margin-bottom: 0.85rem; }
        h2 { font-size: 12.5pt; text-transform: uppercase; color: #14532d; margin: 1rem 0 0.45rem; border-bottom: 1px solid #8ea894; padding-bottom: 0.15rem; }
        ...
        .pdf-footer { position: fixed; bottom: 0.6cm; left: 0; right: 0; text-align: center; font-size: 9.7pt; color: #4a4a4a; border-top: 1px solid #bdbdbd; padding-top: 0.2cm; }
    ...
    <body>
      <section class="cover-page">
        ...
        <div class="cover-footer">Quevedo - Ecuador<br/>${anio}</div>
      </section>
      <main class="doc-main">
  ...
  `;
    ...
    }, 500); // ✅ Aumentado a 500ms para dar tiempo a cargar el base64
  }
  ...
  private sanitizeRichHtml(rawHtml: string): string {
    ...
    doc.querySelectorAll('*').forEach((el) => {
      [...el.attributes].forEach((attr) => {
        ...
        if ((name === 'href' || name === 'src') && /^javascript:/i.test(value.trim())) { el.removeAttribute(attr.name); return; }
