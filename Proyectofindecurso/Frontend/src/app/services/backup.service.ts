...
export interface BackupDestinationRequest extends BackupDestinationDto {
  azureKey?:          string;   // solo en request, nunca en response
...
export class BackupService {
  ...
  probarPostgres(host: string, port: number, usuario: string, password: string): Observable<TestResultado> {
    return this.http.post<TestResultado>(`${this.API_URL}/test/postgres`, { host, port, usuario, password });
  }

  listarDatabases(host: string, port: number, usuario: string, password: string): Observable<{ exitoso: boolean; databases: string[]; mensaje?: string }> {
  ...
  listarCarpetasDrive(destinationId: number): Observable<{ exitoso: boolean; carpetas: { id: string; name: string }[]; mensaje?: string }> {
    return this.http.get<any>(`${this.API_URL}/oauth/google/folders/${destinationId}`);
  }
