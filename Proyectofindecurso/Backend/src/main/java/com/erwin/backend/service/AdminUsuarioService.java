...
public class AdminUsuarioService {
    ...
    // ---------------------------------------------------
    // CREAR USUARIO ✅ Usa sp_crear_usuario_v4 (multi-rol, roles de distintas bases)
    // ---------------------------------------------------
    @Transactional
    public UsuarioAdminDto crear(UsuarioCreateRequest req) {
        ...
        // PASSWORD APP → BCrypt
        String passwordHash = passwordEncoder.encode(req.getPasswordApp().trim());
        ...
        // ✅ Determinar idRolAppPrincipal
        // Si el frontend lo envía lo usamos; si no, usamos el primero del array
        Integer idRolAppPrincipal = req.getIdRolAppPrincipal();
        if (idRolAppPrincipal == null && idsRolApp.length > 0) {
            idRolAppPrincipal = idsRolApp[0];
        }

        // ✅ LLAMAR A sp_crear_usuario_v4 (12 parámetros, soporta multi-rol cross-base)
        Integer newId = usuarioSpRepo.crearUsuarioV4(
                req.getCedula().trim(),
        ...
        UsuarioAdminDto dto = usuarioSpRepo.obtenerPorId(newId);
    ...
    public UsuarioAdminDto editar(Integer id, UsuarioUpdateRequest req) {
        ...
        usuarioSpRepo.editarUsuarioV3(id, nombres, apellidos, req.getActivo(), password, idsRolApp);

        UsuarioAdminDto dto = usuarioSpRepo.obtenerPorId(id);
        if (dto == null) throw new RuntimeException("Usuario no existe");
    ...
    private void validarCreate(UsuarioCreateRequest req) {
        ...
        if (vacio(req.getPasswordApp())) throw new RuntimeException("Password App requerida");
        ...
        if (req.getIdsRolApp() == null || req.getIdsRolApp().length == 0) {
            throw new RuntimeException("Seleccione al menos un Rol del Aplicativo");
        }
    }
